...
import { createDoctor, requestDoctorEmailVerification } from '../../shared/api/doctorApi';
...
const EMAIL_COOLDOWN_KEY = 'doctorCreate.emailCooldown.v1';
...
export default function DoctorCreatePage() {
    ...
    async function handleCreateDoctor(e: React.FormEvent) {
        ...
        try {
            ...
            setMessage(result.message || 'Лікаря створено');
        } catch (err) {
            setError(err instanceof Error ? err.message : 'Не вдалося створити лікаря');
        } finally {
            setSaving(false);
        }
    }
    ...
    return (
        <div className="page-shell doctor-create-page">
            <div className="container doctor-create-page__container">
                <div className="doctor-create-page__content">
                    {error && (
                    ...
                    <section className="doctor-create-page__card">
                        <h1 className="doctor-create-page__title">СТВОРЕННЯ ЛІКАРЯ</h1>
                        <p className="doctor-create-page__subtitle">Доступно для ADMIN та SUPER_ADMIN.</p>

                        {!isAllowed ? (
                            <div className="doctor-create-page__blocked">Недостатньо прав.</div>
                        ) : (
                            <form className="doctor-create-page__form" onSubmit={handleCreateDoctor}>
                                ...
                                <div className="doctor-create-page__verify-row">
                                    <button
                                        type="button"
                                ...
                                <button className="doctor-create-page__submit" type="submit" disabled={saving}>
                                    {saving ? 'СТВОРЕННЯ...' : 'СТВОРИТИ ЛІКАРЯ'}
                                </button>
                            </form>
                        )}
                    </section>
            ...
            {isPhoneModalOpen && telegramBotUrl && (
                <div className="doctor-create-page__modal-backdrop">
                    <div className="doctor-create-page__modal">
                        <h2 className="doctor-create-page__modal-title">ПІДТВЕРДЖЕННЯ ТЕЛЕФОНУ</h2>
                        <TelegramQrCard
                            telegramBotUrl={telegramBotUrl}
                            title="QR ДЛЯ ПІДТВЕРДЖЕННЯ ТЕЛЕФОНУ ЛІКАРЯ"
