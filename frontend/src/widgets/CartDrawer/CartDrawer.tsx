...
export default function CartDrawer({ isOpen, items, totalUah, onClose, onRemove, onClear, onBook }: Props) {
    ...
    return (
        <>
            {isOpen && (
                <button type="button" className="cart-drawer__backdrop" onClick={onClose} aria-label={tx('cart.close', 'Закрити кошик')} />
            )}
            <aside className={`cart-drawer ${isOpen ? 'is-open' : ''}`}>
                <div className="cart-drawer__header">
                    <div>
                        <h2>{tx('cart.title', 'Кошик')}</h2>
                        <p>{items.length ? tx('cart.selectedServices', 'Обрані послуги для запису') : tx('cart.empty', 'Кошик порожній')}</p>
                    </div>
                    <button type="button" className="cart-drawer__close" onClick={onClose}>✕</button>
                </div>
                <div className="cart-drawer__body">
                    {items.length === 0 ? (
                        <div className="cart-drawer__empty">{tx('cart.addFromHome', 'Додайте послуги на головній сторінці.')}</div>
                    ) : (
                        <div className="cart-drawer__list">
                            {items.map((item) => {
                                ...
                                return (
                                    <div key={item.cartItemId} className="cart-drawer__item">
                                        <div className="cart-drawer__item-main">
                                            <strong>{serviceName || tx('cart.service', 'Послуга')}</strong>
                                            {categoryName ? <p>{tx('cart.category', 'Категорія')}: {categoryName}</p> : null}
                                            <p>
                ...
                <div className="cart-drawer__footer">
                    ...
                    <div className="cart-drawer__actions">
                        <button type="button" onClick={onClear} disabled={!items.length}>{tx('cart.clear', 'Очистити')}</button>
                        <button type="button" onClick={onBook} disabled={!items.length}>{items.length > 1 ? tx('cart.toSmartBooking', 'До запису') : tx('cart.toBooking', 'До запису')}</button>
                    </div>
                </div>
            </aside>
        </>
